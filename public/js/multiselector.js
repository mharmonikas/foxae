            ...
            this.each(function() {
                ...
                $(this).on('click', options.selector, function(e) {
                    ...
                    if (lastSelected.length > 0 && e.shiftKey && !options.disableShift)
                    {
                        ...
                        $(parent).children('.active').removeClass('active').removeClass('ms-selected');

                        list.slice(Math.min(lastIndex, curIndex), Math.max(lastIndex, curIndex) + 1).addClass(options.selectedElementClass).addClass('active');
                        options.onSelectionEnd(getSelected(parent), parent, this, e);
                        return;
                    }

                    $(parent).children(options.selector).removeClass('active');
