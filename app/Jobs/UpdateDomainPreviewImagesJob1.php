    ...
    private function addWatermark($dbImage, $watermarkImage, $backgrounds)
    {
        ...
        foreach ($backgrounds as $background) {
            ...
            try {
                ...
                if(!$backgroundImage) {
                    Log::info("Background image with ID {$background->bg_id} and background {$background->background_img} is missing.");
                }
...
//               $image->save(public_path('upload/destination-image-'.time().'.png'));
    ...
    {
        return Image::make(public_path('background/'.$background->background_img));
    }
    ...
    private function assureDirectoryExists($destinationPath)
    {
