...
public function index(){
	...
	if(isset($_GET['VchRaceTagID'])){
			if($_GET['VchRaceTagID'] != "" && $_GET['VchRaceTagID'] != 0 ){
				$getallvideo = $getallvideo->where("tbl_Videotagrelations.VchRaceTagID",$_GET['VchRaceTagID']);
			}
		}
...
$Watermark='';
	if($tumbvideo->EnumType == 'V'){
	 ...
	 if($tumbvideo->vchcacheimages == "" && $tumbvideo->EnumType != 'V'){ 
		...
		$dataarray = array('vchcacheimages'=>DIR_CACHE.$thumb);
		DB::table('tbl_Video')->where('IntId', $tumbvideo->IntId)->update($dataarray);
	 } 
